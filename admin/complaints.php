...
if (isset($_GET['ajax_replies'])) {
    ...
    $st->close();
    echo json_encode(['success'=>true,'replies'=>$rows]); exit();
}
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['ajax_send_reply'])) {
    ...
    if (empty($msg)) { echo json_encode(['success'=>false,'error'=>'Empty message']); exit(); }
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['update_complaint'])) {
    ...
    $upd->bind_param("ssi",$status,$resp,$cid); $upd->execute(); $upd->close();
...
$sql = "SELECT c.complaint_id,c.complaint_date,c.title,c.description,c.status,c.admin_response,c.created_at,t.name AS tenant_name,t.unit_number"
     .($hasUrgent?",c.urgent":',0 AS urgent')
     ." FROM complaints c JOIN tenants t ON c.tenant_id=t.tenant_id ORDER BY c.complaint_date DESC";
            ...
            <div class="card <?= $isUrgent?'border-danger':'' ?>">
                <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                    <div>
                        ...
                        <span class="badge bg-info ms-1" id="badge-<?= $cid ?>"><i class="fas fa-comments me-1"></i><span><?= $rc ?></span></span>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <button class="btn btn-sm btn-dark" data-bs-toggle="collapse" data-bs-target="#thread<?= $cid ?>" data-cid="<?= $cid ?>">
                    ...
                    <div class="card-body border-top pt-3">
                        ...
                        <div class="thread-input-row">
                            <input type="text" class="form-control thread-reply-input"
                                   id="replyInput<?= $cid ?>" placeholder="Type your reply..." data-cid="<?= $cid ?>">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="form-text">Primary response on tenant's card. Use Thread for follow-up messages.</div>
                            </div>
                        </div>
...
(function(){
    ...
    if(srch) srch.addEventListener('input',function(){
        ...
        document.querySelectorAll('.complaint-card-wrapper').forEach(function(c){ c.style.display=c.textContent.toLowerCase().includes(q)?'':'none'; });
    });
    ...
    function bubble(r){
        ...
        var time=new Date(r.created_at.replace(' ','T')).toLocaleString('en-PH',{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'});
    ...
    function loadReplies(cid){
        ...
        fetch('complaints.php?ajax_replies=1&complaint_id='+cid)
            ...
            .then(function(data){
                ...
                if(data.replies&&data.replies.length>0){ data.replies.forEach(function(r){box.appendChild(bubble(r));}); }
                else{ box.innerHTML='<p class="text-muted text-center small py-3 mb-0"><em>No messages yet. Start the conversation.</em></p>'; }
                scrollBot(box); loaded[cid]=true;
            })
            .catch(function(){ var box=document.getElementById('threadMessages'+cid); if(box) box.innerHTML='<p class="text-danger text-center small py-3">Failed to load.</p>'; });
    }
    ...
    function send(cid){
        ...
        inp.disabled=true; btn.disabled=true; btn.innerHTML='<i class="fas fa-circle-notch fa-spin"></i>';
        var fd=new FormData();
        fd.append('ajax_send_reply','1'); fd.append('complaint_id',cid);
        fd.append('reply_message',msg); fd.append('csrf_token',csrfToken);
        fetch('complaints.php',{method:'POST',body:fd})
            ...
            .then(function(data){
                if(data.success){
                    ...
                    box.appendChild(bubble(data.reply)); scrollBot(box); inp.value='';
