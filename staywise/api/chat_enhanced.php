...
try {
    ...
    if (preg_match('/\b(pay|payment|rent|bill|due|balance)\b/', $msgLower)) {
        ...
        $confidence = 0.85;
        $quickAction = ['label' => 'View Payments', 'url' => '/StayWise/tenant/payments.php'];
    } elseif (preg_match('/\b(complaint|maintenance|repair|fix|broken|leak|damage)\b/', $msgLower)) {
        $intent = 'maintenance_request';
        $confidence = 0.85;
        $quickAction = ['label' => 'Submit Complaint', 'url' => '/StayWise/tenant/complaints.php'];
    } elseif (preg_match('/\b(predict|forecast|upcoming|prevention|preventive)\b/', $msgLower)) {
        $intent = 'prediction_inquiry';
        $confidence = 0.80;
        if ($userRole === 'admin') {
            $quickAction = ['label' => 'View Insights', 'url' => '/StayWise/admin/ai_insights.php'];
        }
    }
