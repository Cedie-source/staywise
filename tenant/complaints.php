...
if (!isset($_SESSION['user_id'])||$_SESSION['role']!='tenant') { header("Location: ../index.php"); exit(); }
...
try {
    ...
    if($ep){$ep->execute();$r=$ep->get_result()->fetch_assoc();if($r)$emergency_phone=$r['setting_value'];$ep->close();}
} catch(Throwable $e){}
...
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['submit_complaint'])){
    ...
    $cdate=$_POST['complaint_date']??date('Y-m-d'); $urg=isset($_POST['urgent'])?1:0;
...
.thread-input-row textarea{flex:1;border-radius:12px;padding:.4rem .9rem;resize:none;}
.thread-input-row button{border-radius:12px;padding:.5rem 1rem;white-space:nowrap;flex-shrink:0;}
                        ...
                        <div class="card-body complaint-item">
                            <div class="d-flex justify-content-between align-items-start mb-2">
                                ...
                                <div class="d-flex gap-1 align-items-center flex-wrap">
                                    ...
                                    <span class="badge bg-info" id="badge-<?=$cid?>"><i class="fas fa-comments me-1"></i><span id="badgeCount-<?=$cid?>"><?=$rc?></span></span>
                                </div>
                            </div>
...
(function(){
    ...
    function makeBubble(r){
        ...
        var name=r.full_name||(isAdmin?'Admin':'You');
        ...
        d.innerHTML='<div class="bubble-meta"><i class="fas '+(isAdmin?'fa-user-shield':'fa-user')+' me-1"></i>'+esc(name)
            +' <span class="badge '+(isAdmin?'bg-secondary':'bg-light text-dark')+' ms-1" style="font-size:.63rem;">'+(isAdmin?'Admin':'You')+'</span></div>'
    ...
    function showErr(cid,msg){var el=document.getElementById('threadErr'+cid);if(el){el.textContent=msg;el.style.display=msg?'block':'none';}}

    function getState(cid){if(!threadState[cid])threadState[cid]={loaded:false,loading:false};return threadState[cid];}
    ...
    function loadReplies(cid){
        ...
        fetch('complaints.php?ajax_replies=1&complaint_id='+cid,{credentials:'same-origin'})
            ...
            .catch(function(){
                var box=document.getElementById('threadBox'+cid);
                if(box)box.innerHTML='<p class="text-danger text-center small py-3">Could not load.</p>';
                state.loading=false;
            });
    }
    ...
    document.querySelectorAll('.thread-toggle-btn').forEach(function(btn){
        btn.addEventListener('click',function(){
            ...
            var isOpen=thread.classList.contains('show');
            if(!isOpen){ // about to open
                setTimeout(function(){loadReplies(cid);},100);
            }
        });
    });
